...
- Imágenes: El `pom.xml` ya incluye `cloudinary-http5` y `cloudinary-core`, pero la subida multipart aún no está implementada. `ImagenProductoController` recibe `@RequestBody ImagenRequest` (JSON) con `urlImagen`. Si se implementa multipart/form-data con `@RequestPart`, el flujo funcionará con Cloudinary.